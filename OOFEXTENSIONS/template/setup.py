...
libname  = 'oofextension_' + modulename
swigfilename = modulename + '.swg'
srcdir = modulename
swigdestdir = modulename + '_SWIG'
cfilepath = os.path.join(modulename, modulename+'.C')
...
library = oof2extutils.SharedLibrary(
    name=libname,
    sources=[cfilepath])
...
extension, package = oof2extutils.get_swig_ext(
    srcdir  = srcdir,
    srcfile = swigfilename,
    destdir = swigdestdir,
    libraries= [libname])

distutils.core.setup(
    # The first arguments here are pretty much self-explanatory and
    # nonconsequential, unless you're going to publish this code.
    name = modulename,
    version = "unknown",

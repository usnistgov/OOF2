    ...
    def make_stiffness(self):
        ...
        for e in self.elementlist:
            ...
            for g in e.gausspts():
                ...
                dofval = e.dof("Displacement", g.xi,g.zeta,g.mu)
                dofderivs = e.dofdx("Displacement", g.xi,g.zeta,g.mu)
                GPC = GP_DEP(self)
                DepC = GPC.umat(xyz,SHP)
                Dep = DepC[0]
                CS = DepC[1]
                Dep_gpt.append(Dep)
...
#********
    ...
    def _flux_contrib(self, element, rndx, eqn, eqndx, gpt,xyz,SHP):
        ...
        for j in range(3):
            res -= element.dshapefn(rndx,j,gpt.xi,gpt.zeta,gpt.mu)*\
                   eqn.flux.value(eqndx,j,pos,dofval,dofderivs,xyz,SHP)
        return res
...
###################### post-proccesing ###########################
    def evaluate_ss(self):
        ...
        for e in m.elementlist:
            ...
            for g in e.gausspts():
                ...
                dvalstrain = self.CastressTstrain(e,g,xyz,SHP)[0]
                dvalstress = self.CastressTstrain(e,g,xyz,SHP)[1]
    ...
    def CastressTstrain(self, element,gpt,xyz,SHP):

        pos = element.frommaster(gpt.xi,gpt.zeta,gpt.mu)
        dofderivs = element.dofdx("Displacement", gpt.xi,gpt.zeta,gpt.mu)
        CaT = self.Cavalue(xyz,SHP)
...
#-------------- elasto-plastic modulus zt the gauss points ----------
class Flux:
...
class GP_DEP(Flux):
    ...
    def umat(self,xyz,SHP):

        self.cijkl = Cijkl(lmbda=1.0,mu=0.5)
        FG = self.calc_F(xyz,SHP)            # F at gausspoint
        E_lag = self.calc_E_lag(FG)            # Euler-Lagrange strain
        S = self.SPK_stress(E_lag)             # 2nd PK stress
        Cauchy = self.Chuchy_stress(S,FG)      # Cauchy stress
        N = self.cijkl
...
####################### Calculate the Chuchy stress  ############################
    ...
    def Chuchy_stress(self,S_2PK,Fe):
...
#### End of drivative of 1PK stress to gradient of displacement ######
    ...
    def value(self,i,j,pos,dofval,dofderivs,xyz,SHP):

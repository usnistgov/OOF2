...
1.1 Getting more control over the build
   ...
   --enable-openmp   Turn on OpenMP code in OOF2 for parallel execution.
                     This will only work if your compiler supports
                     OpenMP, which the Macintosh clang compiler does
                     not.  (In OOF2 2.1.12 only the matrix construction
                     and pixel autogroup operations are parallelized.)

    Prior to version 2.1.12, setup.py accepted the arguments
    --skip-swig and --with-swig to control swig, which is used to
    generate some of the OOF2 code.  The OOF2 distribution now
    includes its own copy of swig, and those arguments are
    unnecessary.

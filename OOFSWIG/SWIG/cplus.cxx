...
/*******************************************************************************
 * $Header: /users/langer/FE/CVSoof/OOF2/OOFSWIG/SWIG/cplus.cxx,v 1.1 2014/06/25 18:40:50 lck Exp $
 *
 * File : cplus.cxx
 *
...
void cplus_declare_const(const char *,const char *, DataType *,const char *);
...
static void add_local_type(const char *type, char *classname) {
  String str;
...
public:
  ...
  CPP_destructor(char *n,const char *i) {
    name = copy_string(n);
    iname = copy_string(i);
    new_method = AddMethods;
    de = 0;
...
public:
  char       *value;
  DataType   *type;
  CPP_constant(const char *n,const char *i, DataType *t,const char *v) {
    name = copy_string(n);
    iname = copy_string(i);
    type = new DataType(t);
    value = copy_string(v);
...
public:
  ...
  CPP_class(const char *name,const char *ctype) {
    CPP_class *c;
  ...
  CPP_member *search_member(const char *name) {
    CPP_member *m;
    char *c;
    m = members;
    while (m) {
  ...
  static CPP_class *search(const char *name) {
    CPP_class *c;
    c = classlist;
    if (!name) return 0;
    while (c) {
...
char *cplus_base_class(const char *name) {
...
void cplus_open_class(const char *name, char *rname,const char *ctype) {
  ...
  if (name) {
	...
	typeeq_derived(temp,(char*)name);       // Map "struct foo" to "foo"
	typeeq_derived((char*)name,temp);       // Map "foo" to "struct foo"
      }
    }
  }
...
void cplus_declare_const(const char *name,const char *iname, DataType *type,const char *value) {

  const char *temp_iname;
  ...
  update_symbol(const_cast<const char*>(name), type, value);
...
void cplus_emit_destructor(char *classname, char *classtype, char *classrename, 
			   char *mname,const char *mrename, int mode)
{
    Parm *p;
    DataType *type;
    ParmList *l;
...
void cplus_emit_constructor(char *classname, char *classtype, char *classrename,
                            char *mname,const char *mrename, ParmList *l, int mode)
{
    Parm *p;
    int i;
    DataType *type;
...
void cplus_emit_variable_get(char *classname, char *classtype, char *classrename,
			     char *mname,const char *mrename, DataType *type, int mode) {
...
void cplus_emit_variable_set(char *classname, char *classtype, char *classrename,
	...
	if (tm) {
      ...
      if (mrename) 
	p->name = const_cast<char *>(mrename);
      else
	p->name = mname;
      l->insert(p,0);
      p = new Parm(0,0);

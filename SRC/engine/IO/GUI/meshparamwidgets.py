...
# MeshFieldLister is used as the meshfunc argument to
# FieldParameterWidget.  It determines which Fields will be listed in
# the widget.
...
class MeshFieldLister:
    ...
    def __call__(self, meshctxt):
        ...
        for fld in meshctxt.all_compound_subproblem_fields():
            if self.compound:
                yield fld
            if self.timederivative and fld.time_derivative() in tdfields:
                yield fld.time_derivative()
            if self.outofplane and fld.out_of_plane() in oopfields:
                yield fld.out_of_plane()
            if (self.outofplane and self.timederivative and
                fld.out_of_plane_time_derivative() in tdfields):
                yield fld.out_of_plane_time_derivative()

...
def parallel_skel_info_query(menuitem, targetname, position, skeleton):
    ...
    homogeneity=%s""" % (sindex,
                         ",".join(["Node %d at (%g, %g)" %
                                   (obj.index, obj.position().x, obj.position().y)
                                      for obj in sgmt.get_nodes()]),
                         ",".join(["Element %d" % obj.index
                                   for obj in sgmt.getElements()]),
                         length,
                         homogval)
    ...
    material=%s""" % (eindex,
                      etype,
                      ",".join(["Node %d at (%g, %g)" % (obj.index,
                                                         obj.position().x,
                                                         obj.position().y)
                                for obj in elem.nodes]),
                      ",".join(["Segment %d, nodes (%d, %d) (length: %g)" %
                                (obj.index, obj.nodes()[0].index,
                                 obj.nodes()[1].index,
                                 obj.length())
    ...
    if skelcontext:
        ...
        try:
            if moveit:
                ...
                #TODO: If the node is shared, the move may be valid in one process

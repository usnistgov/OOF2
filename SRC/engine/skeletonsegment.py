...
class SkeletonSegment(skeletonselectable.SkeletonSelectable):
    ...
    def homogeneity2(self, microstructure):
        ...
        ## TODO: These calls to edgeHomogeneity will both call
        ## CMicrostructure::segmentPixels, but second call could reuse
        ## the results of the first.  Perhaps
...
def segSequence(seglist, startnode=None):
    ...
    for s in seglist:
        n = s.get_nodes()
        adjacency.setdefault(n[0], []).append(s)

        for p in n[0].getPartners():
            adjacency.setdefault(p, []).append(s)

        adjacency.setdefault(n[1], []).append(s)

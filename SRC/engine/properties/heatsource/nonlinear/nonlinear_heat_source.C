...
NonlinearHeatSource::NonlinearHeatSource(const std::string &name, PyObject *reg)
...
int NonlinearHeatSource::integration_order(const CSubProblem *,
					   const Element *el)
  const
{
...
void NonlinearHeatSource::force_value(const FEMesh *mesh,
...
} // NonlinearHeatSource::force_value

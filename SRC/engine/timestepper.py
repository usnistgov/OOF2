...
class StepDriver(registeredclass.RegisteredClass):
    ...
    ## TODO: What about steppers that require matrix evaluation only
...
## TODO CLEANUP: This proliferation of dispatch routines for
...
class NonStaticStepper(object):
    ...
    ## TODO CLEANUP: These rely on the fact that all of the storage
    ...
    def nextStepEstimate(self, time, stepResult):
        ...
        if(stepResult.errorEstimate == 0.0):
            ...
            if(time + newstep < stepResult.endTime):
                ...
                # of 0.9.
                ## TODO: Get rid of the factor.  It shouldn't be
...
class BasicStepDriverParameter(parameter.RegisteredParameter):
    def __init__(self, name, value=None, default=None, tip=None):
        super(BasicStepDriverParameter, self).__init__(
            name, BasicStepDriver, value, default, tip)
    
class AdvancedStepDriverParameter(parameter.RegisteredParameter):
    def __init__(self, name, value=None, default=None, tip=None):
        super(AdvancedStepDriverParameter, self).__init__(
            name, StepDriver, value, default, tip)
    
#=--=##=--=##=--=##=--=##=--=##=--=##=--=##=--=##=--=##=--=##=--=##=--=#

# TODO: Move RKF45 to its own file, and finish writing it.

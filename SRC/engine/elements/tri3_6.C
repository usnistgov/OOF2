...
public:
		       ...
		       "Subparametric 6 noded triangle with linear interpolation for positions and quadratic interpolation for fields", 6, 3)
  {
    ...
    pn = addProtoNode(MasterCoord(1, 0)); // map node 0, func node 0
    pn->set_mapping();
    pn->set_func();
    pn->set_corner();
    pn->on_edge(0);
    ...
    pn = addProtoNode(MasterCoord(0.5, 0.5)); // func node 1
    pn->set_func();
    pn->on_edge(0);

    pn = addProtoNode(MasterCoord(0, 1)); // map node 1, func node 2
    pn->set_mapping();
    pn->set_func();
    pn->set_corner();
    pn->on_edge(0);
    ...
    pn = addProtoNode(MasterCoord(0, 0.5)); // func node 3
    pn->set_func();
    pn->on_edge(1);

    pn = addProtoNode(MasterCoord(0,0)); // map node 2, func node 4
    pn->set_mapping();
    pn->set_func();
    pn->set_corner();
    pn->on_edge(1);
    ...
    pn = addProtoNode(MasterCoord(0.5, 0)); // func node 5
    pn->set_func();
    pn->on_edge(2);

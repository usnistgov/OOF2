// -*- C++ -*-
// $RCSfile: quad4_8.C,v $
// $Revision: 1.3 $
// $Author: langer $
// $Date: 2014/09/27 21:41:06 $

/* This software was produced by NIST, an agency of the U.S. government,
 * and by statute is not subject to copyright in the United States.
 * Recipients of this software assume all responsibilities associated
 * with its operation, modification and maintenance. However, to
 * facilitate maintenance we ask that before distributing modified
 * versions of this software, you first contact the authors at
 * oof_manager@nist.gov. 
 */

// Eight node quadrilateral element

#include <oofconfig.h>
#include "engine/masterelement.h"
#include "quad4shapefunction.h"
#include "quad8shapefunction.h"
#include <math.h>


// Master element for the eight node quadrilateral.
//
//                      (0, 1)
//  (-1, 1)  6------------5-----------4  (1, 1)
//           |                        |
//           |                        |
//           |                        |
//           |                        |
//  (-1, 0)  7                        3  (1, 0)
//           |                        |
//           |                        |
//           |                        |
//           |                        |
//  (-1,-1)  0------------1-----------2  (1,-1)
//                      (0,-1)        


class Quad8MasterElement : public QuadrilateralMaster {
public:
  Quad8MasterElement()
    : QuadrilateralMaster("Q4_8",
			  "Subparametric 8 noded quadrilateral with bilinear interpolation for positions and quadratic interpolation for fields", 8, 4)
  {
    shapefunction = new Quad8ShapeFunction(*this);
    mapfunction = new Quad4ShapeFunction(*this);

    ProtoNode *pn;

    // 0
    pn = addProtoNode(MasterCoord(-1, -1));
    pn->set_mapping();
    pn->set_func();
    pn->set_corner();
    pn->on_edge(0);
    pn->on_edge(3);

    // 1
    pn = addProtoNode(MasterCoord(0, -1));
    pn->set_func();
    pn->on_edge(0);

    // 2
    pn = addProtoNode(MasterCoord(1, -1));
    pn->set_mapping();
    pn->set_func();
    pn->set_corner();
    pn->on_edge(0);
    pn->on_edge(1);

    // 3
    pn = addProtoNode(MasterCoord(1, 0));
    pn->set_func();
    pn->on_edge(1);

    // 4
    pn = addProtoNode(MasterCoord(1, 1));
    pn->set_mapping();
    pn->set_func();
    pn->set_corner();
    pn->on_edge(1);
    pn->on_edge(2);

    // 5
    pn = addProtoNode(MasterCoord(0, 1));
    pn->set_func();
    pn->on_edge(2);

    // 6
    pn = addProtoNode(MasterCoord(-1, 1));
    pn->set_mapping();
    pn->set_func();
    pn->set_corner();
    pn->on_edge(2);
    pn->on_edge(3);

    // 7
    pn = addProtoNode(MasterCoord(-1, 0));
    pn->set_func();
    pn->on_edge(3);

    addSCpoint(MasterCoord(-1./sqrt(3.), -1./sqrt(3.)));
    addSCpoint(MasterCoord( 1./sqrt(3.), -1./sqrt(3.)));
    addSCpoint(MasterCoord( 1./sqrt(3.),  1./sqrt(3.)));
    addSCpoint(MasterCoord(-1./sqrt(3.),  1./sqrt(3.)));
  }

  virtual ~Quad8MasterElement() {
    delete shapefunction;
    delete mapfunction;
  }

  int map_order() const {
    return 1;
  }
  int fun_order() const {
    return 2;
  }

};

void quad4_8init() {
  static Quad8MasterElement m;
}

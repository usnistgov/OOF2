			      ...
			      SmallSystem *fluxdata) const
{
  ...
  DoubleVec fieldGradient;
  
  if (*flux == *charge_flux ) {
    // For charge flux, the relevant gradient is the concentration.
    fieldGradient = concentration->gradient(mesh, element, pt);
  }
  else if (*flux == *atom_flux ) {
    // For atom flux, the dependent field gradient is the voltage.
    fieldGradient = voltage->gradient(mesh, element, pt);
  }
  else 
    throw ErrProgrammingError("Unexpected flux", __FILE__,__LINE__);

  DoubleVec flx = -(z_*c) * cndct*fieldGradient;
  for(int i=0; i<3; i++)
  ...
  const
{
  ...
  if (*flux==*charge_flux) {
    ...
    fieldGradient = concentration->gradient(mesh, el, pt);
    ...
    fieldGradient = voltage->gradient(mesh, el, pt);

...
#TODO 3D SOMEDAY: This might be sped up by converting to C++, but for
#now, it seems fast enough.  We'll probably need to convert to C++
#when we convert the canvas.
...
class VoxelPicker:
    ...
    def Pick(self, selectionX, selectionY, selectionZ, renderer):
        ...
        # as our picked point.  This method assumes that the world
        # coordinates correspond to image coordinates.
        ...
        while (lengthTraversed <= rayInCRLength and self.PickedVoxel is None):
            # if currentpoint in bounds, divide by spacing, truncate and return
            if self.InBounds(currentPoint):
                spacing = self.image.GetSpacing()
                self.PickedVoxel = [int(currentPoint[0]/spacing[0]),
                                    int(currentPoint[1]/spacing[1]),
                                    int(currentPoint[2]/spacing[2])]

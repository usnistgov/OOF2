...
bool OOFCanvas::initialized = false;
...
// TODO: One of the reasons for doing this by segments was to avoid an
...
void OOFCanvas::mouse_eventCB(GnomeCanvasItem *item, GdkEvent *event) {
  ...
  PyGILState_STATE state = (PyGILState_STATE) acquirePyLock();
  try {
    switch(event->type) {
    case GDK_BUTTON_PRESS:
      shift = event->button.state & GDK_SHIFT_MASK;
      ctrl = event->button.state & GDK_CONTROL_MASK;
      args = Py_BuildValue((char*) "sddii", "down",
    ...
    case GDK_BUTTON_RELEASE:
      shift = event->button.state & GDK_SHIFT_MASK;
      ctrl = event->button.state & GDK_CONTROL_MASK;
      args = Py_BuildValue((char*) "sddii", "up",
			   event->button.x, -event->button.y, // flip y axis!
			   shift, ctrl);
    ...
    case GDK_MOTION_NOTIFY:
      ...
      args = Py_BuildValue((char*) "sddii", "move",
...
void OOFCanvas::configure_eventCB(GtkAllocation *a) {
  ...
  PyGILState_STATE state = (PyGILState_STATE) acquirePyLock();
  try {
    PyObject *args = Py_BuildValue((char*) "iiii", 
				   (int)(a->x), (int)(a->y), 

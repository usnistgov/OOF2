...
void Burner::burn(const OOFImage3D &image, const ICoord *spark, BoolArray &burned)
{
  // Initialize the data structures.
  int nburnt = 0;
  startcolor = image[spark];
  std::vector<ICoord> activesites; // sites whose neighbors have to be checked
  activesites.reserve(image.sizeInPixels()(0)*image.sizeInPixels()(1)*image.sizeInPixels()(2));
